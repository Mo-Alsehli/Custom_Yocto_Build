...
class FileReceiver {
   public:
    FileReceiver(const std::string& outputName) : outPath_("/home/root/commonapi-filetf/data/client/" + outputName) {
        ensureClientDir();
...
int main(int argc, char** argv) {
    ...
    while (!proxy) {
        ...
        if (!proxy->isAvailable()) {
            std::cout << "[Client] Service not available yet. Retrying..." << std::endl;
    ...
    if (status != CommonAPI::CallStatus::SUCCESS) {
        std::cerr << "[Client] requestFile failed, status=" << static_cast<int>(status) << std::endl;
        return 1;
    }
